...
# docker run --env-file devops/docker/env.list -it --name api_logic_project --rm --net dev-network -p 5656:5656 -p 5002:5002 thomasgpeters/northwind_api_logic:1.0.0 bash

# Start container and run the app
docker run --env-file devops/docker-image/env.list --name api_logic_project --rm --net dev-network -p 5656:5656 -p 5002:5002 thomasgpeters/northwind_api_logic:1.0.0
